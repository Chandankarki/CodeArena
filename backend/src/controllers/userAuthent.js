...
//     res.cookie("token", token, { maxAge: 3600000, httpOnly: true, sameSite: "Lax", secure: false, });
//     res.status(201).json({
//       user: reply,
...
//     // User Schema se delete karna hai
...
const login = async (req,res)=>{
    ...
    try{
      ...
      httpOnly: true,
      secure: false, // true only in HTTPS (localhost is HTTP, so keep false)
      sameSite: "Lax", // Acceptable for same-origin (localhost frontend and backend)
